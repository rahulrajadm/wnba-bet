...
def fetch_team_game_logs(seasons: list[str] = SEASONS) -> pd.DataFrame:
    frames = []
    for season in seasons:
        try:
            log  = leaguegamelog.LeagueGameLog(
                league_id=LEAGUE_ID, season=season, season_type_all_star="Regular Season"
            )
...
def fetch_player_game_logs(seasons: list[str] = SEASONS) -> pd.DataFrame:
    frames = []
    for season in seasons:
        try:
